...
const BooksGrid: React.FC<BooksGridProps> = ({ limit, showFilters = false }) => {
  ...
  useEffect(() => {
    ...
    if (books.length === 0) {
      ...
      fetchBooks().then(() => {
        ...
        console.log(`📚 Books loaded in ${Math.round(loadTime)}ms`);
      });
    }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      console.log('📚 BookGrid component mounted');
    }
    return () => {
      if (typeof window !== 'undefined') {
        console.log('📚 BookGrid component unmounted');
      }
    };
  }, []);
      ...
      <div className="py-12">
        <div className="flex justify-center items-center min-h-[200px]">
          <div className="flex flex-col items-center space-y-4">
            ...
            <p className="text-gray-300 text-sm font-medium">Cargando libros...</p>
          </div>
        </div>
      </div>
    );
